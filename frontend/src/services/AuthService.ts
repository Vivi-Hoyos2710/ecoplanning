...
export async function getLoggedUser(): Promise<User> {
  try {
    ...
    const { data, status } = await axios.get<User>(`http://${REACT_APP_DJANGO_HOST}:8000/auth/users/me/`, config);
    return data;
  } catch (error) {
    return NoUser;
  }
...
export async function getLoginToken(loginForm: LoginFormData): Promise<any> {
  ...
  try {
    const response = await axios.post<LoginToken>(
      `http://${REACT_APP_DJANGO_HOST}:8000/auth/token/login`,
      loginRequest
    );
...
export async function logOut(): Promise<any> {
  try {
    ...
    const response = await axios.post<any>(`http://${REACT_APP_DJANGO_HOST}:8000/auth/token/logout/`, token, config);
