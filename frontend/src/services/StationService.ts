...
export async function getStationList(filters: Filter[]): Promise<Station[]> {
  const response = await axios.get<Station[]>(
    `http://${REACT_APP_DJANGO_HOST}:8000/api/v1/station/`,
...
export async function createStation(station: StationInfo) {
  await axios.post<Station>(`http://${REACT_APP_DJANGO_HOST}:8000/api/v1/station/`, station, getAuthConfig());
}

export async function editStation(id: number, station: StationInfo){
  await axios.put<Station>(`http://${REACT_APP_DJANGO_HOST}:8000/api/v1/station/${id}/`, station);
}

export async function deleteStation(id: number){
  const response = await axios.delete(`http://${REACT_APP_DJANGO_HOST}:8000/api/v1/station/${id}/`);
  return response.data;
}
