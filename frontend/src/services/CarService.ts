...
export async function getCarList(filters: Filter[]): Promise<CarInfo[]> {
  const response = await axios.get<CarInfo[]>(
    `http://${REACT_APP_DJANGO_HOST}:8000/api/v1/car/`,
...
export async function createCar(car: Car) {
  await axios.post<Car>(`http://${REACT_APP_DJANGO_HOST}:8000/api/v1/car/`, car, getAuthConfig());
}
export async function updateCar(car: Car, id: number) {
  await axios.put<Car>(`http://${REACT_APP_DJANGO_HOST}:8000/api/v1/car/${id}/`, car, getAuthConfig());
}
export async function deleteCarById(id: number) {
  const response = await axios.delete(`http://${REACT_APP_DJANGO_HOST}:8000/api/v1/car/${id}/`, getAuthConfig());
  return response.data;
}
